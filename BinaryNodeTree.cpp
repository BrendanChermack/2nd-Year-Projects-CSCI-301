...
BinaryNode<ItemType>* BinaryNodeTree<ItemType>::removeValue( BinaryNode<ItemType>* subTreePtr,
    ...
    if ( subTreePtr->getItem() == target ) // found it, subTreePtr points to the node containing the target item
    {
        subTreePtr = moveValuesUpTree( subTreePtr );
        success = true;
        return subTreePtr;  //subTreePtr still points to the node that originally contains the target item
    }
    else {
...
BinaryNode<ItemType>* BinaryNodeTree<ItemType>::copyTree( const BinaryNode<ItemType>* treePtr ) const {
    ...
    if ( treePtr != nullptr ) {
        ...
        /*
        leftPtr = copyTree(treePtr->getLeftChildPtr());
        rightPtr = copyTree(treePtr->getRightChildPtr());
        newTreePtr->setLeftChildPtr(leftPtr);
        newTreePtr->setRightChildPtr(rightPtr);
        */
    }  // end if
    return newTreePtr;
}  // end copyTre
